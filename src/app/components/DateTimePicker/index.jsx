...
function DateTimePicker({
  ...
  const renderArrDate = useCallback(() => {
    const firstDateOfMonth = new Date(
      (virtualDate ?? new Date()).getFullYear(),
      (virtualDate ?? new Date()).getMonth(),
      1
    );
    const lastDateOfMonth = new Date(
      (virtualDate ?? new Date()).getFullYear(),
      (virtualDate ?? new Date()).getMonth() + 1,
      0
    );
    const rows = [];
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const rowCells = arrDate.slice(startIndex, endIndex).map((day, index) => {
        ...
        const isCorrectDate =
          _virtualDate.getTime() === dayWithoutTime.getTime() && date;
        return (
          <td
            onClick={(e) => {
  ...
  useEffect(() => {
    ...
    if (hourContainerRef.current && minuteContainerRef.current) {
      ...
      const positionHourParent =
        hourContainerRef.current.getBoundingClientRect();
      const positionMinuteParent =
        minuteContainerRef.current.getBoundingClientRect();
      ...
      <div ref={parentRef} className="w-full relative cursor-pointer">
        <input
          ...
          className={`text-sm py-2 px-2 border-1
                    ...
                    !text-[#495057]
                   select-none 
                     cursor-pointer focus:border-[#d2d6da] focus:outline-none
        ...
        <div
          ref={contentRef}
          className={` z-max select-none animation-date-ranger-picker  w-full border border-gray-200 rounded bg-white  overflow-y-hidden overflow-x-hidden ${
        ...
        >
          ...
          >
            ...
            <div className="flex flex-row  justify-center items-center cursor-pointer select-none gap-1">
              {isHaveHour && (
                <>
                  {(selectedTime === "day" || selectedTime === "time") && (
                    ...
                    >
                      {virtualHour > 9 ? virtualHour : "0" + virtualHour}:
                      {virtualMinute > 9 ? virtualMinute : "0" + virtualMinute}
                    </p>
                  )}
                  {selectedTime === "day" && (
              ...
              {(selectedTime === "day" || selectedTime === "month") && (
                <p
                  onClick={() => {
                    setSelectedTime(selectedTime === "month" ? "day" : "month");
                  }}
                  className={`text-base ${
                    selectedTime === "month"
                      ? "text-[#008db9]"
              ...
              {(selectedTime === "day" || selectedTime === "year") && (
                <p
                  onClick={() => {
                    setSelectedTime(selectedTime === "year" ? "day" : "year");
                  }}
                  className={`text-base ${
                    selectedTime === "year"
                      ? "text-[#008db9]"
