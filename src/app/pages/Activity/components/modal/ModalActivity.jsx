...
import {addActivity} from "services/api.service";
import NotifyMessage from "_utils/notify";
import CustomModal from "app/components/Modal/CustomModal";
import {activitySchema} from "_utils/validation";
import slugify from "slugify";
import TinyMCE from "app/components/TinyMCE";
import {updateActivity} from "services/api.service";
import {getActivityById} from "services/api.service";
...
const ModalActivity = ({
	...
	const onSubmit = handleSubmit(async (values) => {
		try {
			if (itemId) {
				const data = await updateActivity(itemId, values);
				...
				const data = await addActivity(values);
				if (data?.data?.status === 1) {
					...
					if (item?.name === "content") {
						return (
							<div key={index} className={item?.width}>
								...
								<TinyMCE
									value={watch("content")}
									onEditorChange={(value) => handleOnChangeInput(value)}
									height="400px"
								/>
							</div>
						);
						...
						return (
							<div key={index} className={item?.width}>
								<MyUploadFile
									...
									isMulti={true}
									errorMessage={
										errors?.images ? errors?.images?.message : undefined
									}
								/>
								{getImages?.length > 0 && (
										...
										{getImages.map((item, index) => (
											<div key={index}>
												<Badge
													...
													color="primary"
													onClick={() => {
														const newImages = getImages.filter(
															(_, idx) => idx !== index
														);
