...
import { createNewLabel } from "services/api.service";
...
function NewLabel({ labels, taskId }) {
	const dispatch = useDispatch();
	const { task } = useSelector((state) => state.tasks);
	const [title, setTitle] = useState("");
	const [isLoading, setIsLoading] = useState(false);
	const [colorSelected, setColorSelected] = useState(
		labelColors.find((itemA) => labels.indexOf(itemA) === -1)
	);
	...
	const handleCreateLabel = async () => {
		try {
			...
			const { data } = await createNewLabel({
				title,
				taskId,
				colorLabel: colorSelected?.colorLabel,
				colorCode: colorSelected?.colorCode,
			});

			if (data.status === 1) {
				const newLabels = [...task?.labels, data?.label];
	...
	return (
		<div className="mt-2 px-1 flex flex-col gap-2 w-full">
			...
			<div className="flex justify-end">
				...
				>
					<span className="font-semibold text-white">Tạo</span>
				</Button>
			</div>
		</div>
	);
