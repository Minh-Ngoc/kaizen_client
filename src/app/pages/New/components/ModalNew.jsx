...
import { newSchema } from "_utils/validation";
import slugify from "slugify";
import TinyMCE from "app/components/TinyMCE";
import { updateNew, getNewById } from "services/api.service";
...
const ModalNew = ({ isOpen, onClose, itemId, onOpenChange, onGetPaging }) => {
	...
	const getThumbnail = watch("thumbnail");
	...
	const onSubmit = handleSubmit(async (values) => {
		try {
			if (itemId) {
				const data = await updateNew(itemId, values);
				...
				const data = await addNew(values);
				if (data?.data?.status === 1) {
